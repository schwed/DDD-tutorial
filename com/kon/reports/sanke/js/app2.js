...
window.utils = {
    ...
    window.DepNavView = Backbone.View.extend({
        ...
        render: function () {
            ...
            for (var c = "", d = 0; b > d; d++)
                a[d].get("type_id") >= 1e3 && a[d].get("type_id") <= 2e3 && (c != a[d].get("type_nm").toString() && $(".nav-list", this.el).append('<li class="nav-header">' + a[d].get("type_nm").toString() + "</li>"), c = a[d].get("type_nm").toString(), $(".nav-list", this.el).append(new DepNavItemView({
                    model: a[d],
...
window.DepTreeHeadView = Backbone.View.extend({
    ...
    window.GlossaryRootView = Backbone.View.extend({
        ...
        render: function () {
            ...
            for (var c = "", d = 0; b > d; d++)
                c != a[d].get("type_nm").toString() && $(".nav-list", this.el).append('<li class="nav-header">' + a[d].get("type_nm").toString() + " GLOSSARY</li>"), c = a[d].get("type_nm").toString(), "" != a[d].get("object_nm").toString() && $(".nav-list", this.el).append('<li><a href="#glossary/' + a[d].get("object_id").toString() + '" title="' + a[d].get("object_ds") + '""><i class="icon-conceptsystem"></i>' + a[d].get("object_nm").toString().substr(0, 25) + "(" + a[d].get("glossary_cnt").toString() + "/" + a[d].get("concept_cnt").toString() + ")</a></li>")
        }
    }),
    window.GlossaryTreeView = Backbone.View.extend({
        ...
        render: function () {
            ...
            for (var c = "", d = 0; b > d; d++)
                c != a[d].get("type_nm").toString() && $(".nav-list", this.el).append('<li class="nav-header">' + a[d].get("type_nm").toString() + "</li>"), c = a[d].get("type_nm").toString(), "" != a[d].get("object_nm").toString() && $(".nav-list", this.el).append(new GlossaryTreeItemView({
                    model: a[d],
    ...
    window.GlossaryTreeItemView = Backbone.View.extend({
        ...
        render: function () {
            var a = this.model.get("object_id").toString(),
                b = this.model.get("object_nm").toString(),
                c = (this.model.get("type_nm").toString(), "icon-conceptsystem icon-black"),
    ...
    window.ObjectRootView = Backbone.View.extend({
        ...
        render: function () {
            ...
            for (var c = "", d = 0, e = "", f = 0, g = 0; b > g; g++)
                "" != e && c != a[g].get("gen_type_nm").toString() && $(".list", this.el).append('<li class="nav-header">' + a[g].get("gen_type_nm").toString() + "</li>"), "" != e && e != a[g].get("type_nm").toString() && ($(".list", this.el).append(new ObjectRootItemView({
                    type_id: d,
                    type_nm: e,
                    type_cnt: f
                }).render().el), f = 0), f++, c = a[g].get("gen_type_nm").toString(), e = a[g].get("type_nm").toString(), d = a[g].get("type_id").toString();
    ...
    window.ObjectTreeView = Backbone.View.extend({
        ...
        render: function () {
            ...
            for (var c = "", d = 0; b > d; d++)
                c != a[d].get("type_nm").toString() && $(".nav-list", this.el).append('<li class="nav-header">' + a[d].get("type_nm").toString() + "</li>"), c = a[d].get("type_nm").toString(), $(".nav-list", this.el).append(new ObjectTreeItemView({
                    model: a[d],
                    objectId: this.options.objectId
                }).render().el);
            return this
            ...
            d.fetch({
                success: function () {
                    if (d.get("child_cnt") > 0)
                        var b = d.get("object_id"), c = d.get("parent_object_nm");
                    else
                        var b = d.get("parent_object_id"), c = "BACK";
            ...
            g.fetch({
                success: function () {
                    if (g.get("child_cnt") > 0)
                        var a = g.get("object_id"), e = g.get("parent_object_nm");
                    else
                        var a = g.get("parent_object_id"), e = "BACK";
...
utils.loadTemplate(["HomeView", "HeaderView", "SubHeaderView", "AboutView", "ObjectPageView", "ObjectHomeView", "ObjectView", "PropertyItemView", "ObjectEditView", "GlossaryHomeView", "DepAppView", "DepAppHomeView", "DepSubHeadView", "DepHeadView", "DepItemView", "DepTreeHeadView", "DepTreeView", "DepGraphHeadView", "DepGraphView"], function () {
    ...
    d3.sankey = function () {
        ...
        function c() {
            function a(d) {
                if (d.index = c++, d.lowIndex = d.index, d.onStack = !0, b.push(d), d.sourceLinks && (d.sourceLinks.forEach(function (b) {
                        var c = b.target;
    ...
    !function (a) {
        ...
        var b = '    <button class="dropdown-checkbox-toggle" data-toggle="dropdown" href="#">Dropdown trigger </button>    <div class="dropdown-checkbox-content">      <div class="dropdown-checkbox-header">        <input class="checkbox-all" type="checkbox"><input type="text" placeholder="Search" class="search"/>      </div>      <ul class="dropdown-checkbox-menu"></ul>    </div>',
            ...
            f = function (c, d) {
                ...
                "object" == typeof d && (this.$element.text(d.title), this.$element.addClass(d.btnClass), this.autosearch = d.autosearch, this.elements = d.data || [], this._sort = d.sort || this._sort, this.sortOptions = d.sortOptions, this.hideHeader = d.hideHeader || void 0 === d.hideHeader ? !0 : !1, this.templateButton = d.templateButton, this.showNbSelected = d.showNbSelected || !1),
                    this.$element.append(e),
                this.templateButton && (this.$element.remove(), this.$parent.prepend(this.templateButton), this.$element = this.$parent.find(".dropdown-checkbox-toggle")),
        ...
        f.prototype = {
            ...
            _refreshCheckboxAll: function () {
                var b,
                    c = this.$element.parents(".dropdown-checkbox").find("ul li input[type=checkbox]");
                c.each(function () {
